...
public class LLVMUnixSdkType extends SdkType {
    public static final String ROOT_DIR = "/usr/local";
    private static final String DEAULT_NAME = "LLVMUnixSdk";
    private static final LLVMUnixSdkType INSTANCE = new LLVMUnixSdkType();
    private static final Predicate<String> IS_VERSION = Pattern.compile("[0-9]\\.[0-9]\\.[0-9].*").asPredicate();


    public LLVMUnixSdkType() {
        this(DEAULT_NAME);
    }

    public LLVMUnixSdkType(String name) {
        super(name);
    }

    static LLVMUnixSdkType getInstance() {
        return INSTANCE;
    }
